...
device_count=$(python3 -c 'import torch, torch_npu; print(torch.npu.device_count())' 2>/dev/null)
if [[ -z "$device_count" || "$device_count" -lt 1 ]]; then
    device_count=1
fi
if [[ "$node_size" -lt 1 ]]; then
    node_size=$device_count
fi
if [[ "$device_count" -lt "$node_size" ]]; then
    node_size=$device_count
fi
...
# 配置环境变量
for device_id in $(seq 0 $((node_size-1))); do
    msnpureport -g error -d ${device_id}
done
#设置Device侧日志等级为error
msnpureport -g error
#关闭Device侧Event日志
...
log_file=${output_path_dir}/train_${node_size}p_full.log

torchrun --standalone --nnodes=1 --nproc_per_node=${node_size} main.py \
